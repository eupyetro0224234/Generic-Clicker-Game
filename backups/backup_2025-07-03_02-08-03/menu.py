    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.icon_rect.collidepoint(event.pos):
                # Clicar no ícone abre/fecha o menu principal
                if self.controls_menu.visible:
                    # NÃO fecha controles aqui (pra manter aberto só clicando na opção)
                    pass
                else:
                ...
                if rect.collidepoint(event.pos):
                    ...
                    if 0 <= index < len(self.options):
                        ...
                        if selected == "Controles":
                            self.is_open = False
                            # Toggle visibilidade do menu controles
                            self.controls_menu.visible = not self.controls_menu.visible
                    return True
                else:
                    # Não fecha o menu ao clicar fora da área do menu principal
                    return True
