...
class ConfigMenu:
    def __init__(self, screen, window_width, window_height, loading_callback=None):
        ...
        if not os.path.isfile(self.icon_path):
            try:
                # Baixa o ícone e chama callback de progresso
                def reporthook(block_num, block_size, total_size):
                    if total_size > 0 and self.loading_callback:
                        downloaded = block_num * block_size
                        percent = min(100, downloaded * 100 / total_size)
                        self.loading_callback(percent, "Baixando ícone de configurações...")
                urllib.request.urlretrieve(self.icon_url, self.icon_path, reporthook)
