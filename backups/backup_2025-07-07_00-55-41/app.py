    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_r:
                ...
                upgrade_menu.purchased.clear()  # Resetar upgrades comprados
                continue
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if config_menu.settings_menu.visible and config_menu.settings_menu.handle_event(event):
                    print("Clique tratado no settings_menu")
                    event_handled = True
                if config_menu.controls_menu.visible and config_menu.controls_menu.handle_event(event):
                    print("Clique tratado no controls_menu")
                    event_handled = True
                if config_menu.achievements_menu.visible and config_menu.achievements_menu.handle_event(event):
                    print("Clique tratado no achievements_menu")
                    event_handled = True
                
                res = upgrade_menu.handle_event(event, score)
                if isinstance(res, tuple):
                    score, _ = res
                    print("Clique tratado no upgrade_menu, score atualizado:", score)
                    event_handled = True
                elif res:
                    print("Clique tratado no upgrade_menu")
                    event_handled = True

                if config_menu.handle_event(event):
                    print("Clique tratado no config_menu")
                    event_handled = True
                
                if not event_handled:
                    button._update_rect()  # garante retângulo atualizado
                    if config_menu.settings_menu.is_click_allowed(event.button):
                        if button.is_clicked(event.pos):
                            print("Clique detectado no botão principal!")
                            button.click()
                            bonus = upgrade_menu.get_bonus()
                            print(f"Bônus atual: {bonus}")
                            score += bonus
                            click_effects.append(ClickEffect(event.pos[0], event.pos[1], "+1"))
                            tracker.check_unlock(score)
                        else:
                            print("Clique fora do botão principal")
                    else:
                        print("Clique não permitido pelas configurações")
        ...
        score_manager.save_data(
            ...
            list(upgrade_menu.purchased)  # salvo lista de upgrades comprados
        )
