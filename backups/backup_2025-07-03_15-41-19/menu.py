...
class ConfigMenu:
    def __init__(self, screen, window_width, window_height):
        ...
        # Instanciar menus de controles e configurações
        ...
        # Inicializa controles_menu visible conforme o config.json carregado
        self.controls_menu.visible = self.settings_menu.get_option("Controles")

        # Sincroniza mudanças: quando controles_menu.visible mudar, atualiza config
        # Vamos criar um método para sincronizar isso
    ...
    def handle_event(self, event):
        # Primeiro, eventos para menus internos com prioridade
        if self.settings_menu.visible:
            if self.settings_menu.handle_event(event):
                # Sincroniza visibilidade dos controles com opção config
                controles_opcao = self.settings_menu.get_option("Controles")
                self.controls_menu.visible = controles_opcao
                return True
            return False

        if self.controls_menu.visible:
            if self.controls_menu.handle_event(event):
                # Sincroniza opção config "Controles" com estado visível do menu
                self.settings_menu.set_option("Controles", self.controls_menu.visible)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if menu_rect.collidepoint(event.pos):
                    ...
                    if 0 <= index < len(self.options):
                        ...
                        if selected == "Controles":
                            # Alterna menu controles e atualiza config
                    ...
                    # Clique fora do menu fecha só o menu principal, não os submenus
                    self.is_open = False
                    return True
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                if self.controls_menu.visible:
                    ...
                    self.settings_menu.set_option("Controles", False)
                    self.settings_menu.save_config()
                    return True
                if self.is_open:
                    self.is_open = False
