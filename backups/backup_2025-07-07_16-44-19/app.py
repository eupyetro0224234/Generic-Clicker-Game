...
def main():
    ...
    # Instancia o console (visível False por padrão)
    console = Console(screen, WIDTH, HEIGHT)
    console.visible = False
    console.set_score_accessors(lambda: score, lambda val: set_score(val))
    ...
    while running:
        ...
        for event in pygame.event.get():
            # Quando o ExitHandler estiver ativo
            if config_menu.exit_handler.active:
                config_menu.exit_handler.handle_event(event)

                # Se digitou "console" e apertou Enter no ExitHandler
                if config_menu.exit_handler.user_text.strip().lower() == "console" and event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                    config_menu.enable_console()
                    tracker.unlock_secret("activate_console")  # Desbloqueia a conquista secreta aqui
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_ESCAPE:
                    # Fechar menus e console abertos
                    if console.visible:
                        console.visible = False
                        continue
                    if config_menu.exit_handler.active:
