...
from controles import ControlsMenu  # Importa a classe do arquivo controles.py

class ConfigMenu:
    def __init__(self, screen, window_width, window_height, controls_visible=False):
        self.screen = screen
        ...
        # Instancia o menu de controles, com visibilidade inicial conforme parâmetro
        self.controls_menu = ControlsMenu(screen, window_width, window_height)
        self.controls_menu.visible = controls_visible
    ...
    def draw_menu(self):
        ...
        # Não desenha o menu principal se não estiver aberto ou se o menu controles estiver visível
    ...
    def draw(self):
        # Desenha o menu principal e o menu controles, garantindo que controles fiquem sobrepostos
        self.draw_menu()
        self.controls_menu.draw()

    def handle_event(self, event):
        # Se o menu controles está aberto, passa o evento para ele
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.icon_rect.collidepoint(event.pos):
                # Clicar no ícone fecha menu controles ou abre/fecha menu principal
                if self.controls_menu.visible:
                ...
                if rect.collidepoint(event.pos):
                    ...
                    if 0 <= index < len(self.options):
                        ...
                        if selected == "Controles":
                            # Alterna visibilidade ao clicar repetidamente
                            if self.controls_menu.visible:
                                self.controls_menu.visible = False
                            else:
                                self.controls_menu.visible = True
                            # Fecha o menu principal ao abrir controles
                            self.is_open = False
                    return True
                else:
                    self.is_open = False
