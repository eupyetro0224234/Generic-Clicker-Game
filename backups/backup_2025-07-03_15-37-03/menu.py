...
class ConfigMenu:
    ...
    def handle_event(self, event):
        # Se configurações abertas, processa evento lá e bloqueia outros
        if self.settings_menu.visible:
            if self.settings_menu.handle_event(event):
                return True
            return False

        # Se controles abertos, processa evento lá e bloqueia outros
        if self.controls_menu.visible:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            # Clicou no ícone? Abre/fecha menu
            if self.icon_rect.collidepoint(event.pos):
                self.is_open = not self.is_open
                return True

            # Se menu aberto, checa clique nas opções
            if self.is_open:
                width = 190
                vertical_padding = 6
                full_height = len(self.options) * (self.option_height + self.spacing) - self.spacing + vertical_padding * 2
                ...
                if rect.collidepoint(event.pos):
                    ...
                    if 0 <= index < len(self.options):
                        ...
                        if selected == "Controles":
                            # Toggle controles, sem fechar menu
                            self.controls_menu.visible = not self.controls_menu.visible
                        elif selected == "Configurações":
                            # Toggle configurações, sem fechar menu
                            self.settings_menu.visible = not self.settings_menu.visible
                        # Não fecha o menu principal automaticamente aqui!
                    return True
                else:
                    # Clique fora do menu fecha o menu principal (mas não controles/config)
                    self.is_open = False
                    return True
