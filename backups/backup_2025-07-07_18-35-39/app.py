...
def main():
    ...
    # Inicializa LoadingScreen
    loading = LoadingScreen(screen, WIDTH, HEIGHT)

    # Defina aqui cada etapa real de carregamento do seu jogo:
    etapas = [
        ("Carregando imagens...", lambda: AnimatedButton.preload_assets()),
        ("Inicializando menus...", lambda: None),    # substitua por config_menu.load() se houver
        ("Quase lá...", lambda: None),
        ("Concluído! Iniciando o jogo", lambda: None),
    ]

    for i, (msg, func) in enumerate(etapas, start=1):
        func()  # executa a tarefa real de carregamento
        pct = i / len(etapas) * 100
        loading.draw(pct, msg)
        # não use delay: cada etapa
        # pygame.time.delay(200)

    # Após carregamento, continua o main
