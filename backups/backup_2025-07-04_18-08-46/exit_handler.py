...
class ExitHandler:
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_BACKSPACE:
                ...
                if self.user_text.lower() == "sim":
                    self.fading_out = True
                else:
                    # Limpa o texto para tentar novamente
                    self.user_text = ""
            elif event.key == pygame.K_ESCAPE:
                # Cancelar saída
                self.active = False
            else:
                # Limita o tamanho máximo do texto para caber na caixa
    ...
    def draw(self):
        ...
        # Fundo semi-transparente atrás da caixa
        overlay = pygame.Surface((self.width, self.height), pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 150))
        self.screen.blit(overlay, (0, 0))

        # Caixa de texto maior e alinhada
        pygame.draw.rect(self.screen, self.bg_color, self.input_rect, border_radius=8)

        # Prompt (texto de cima)
