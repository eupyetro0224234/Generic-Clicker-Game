...
from exit_handler import ExitHandler  # Importando nosso ExitHandler corrigido
...
def main():
    ...
    # Instancia o console (invisível por padrão)
    console = Console(screen, WIDTH, HEIGHT)
    console.visible = False

    # Funções para pegar e setar a pontuação
    def get_score():
        return score
    ...
    while running:
        ...
        for event in pygame.event.get():
            if exit_handler.active:
                exit_handler.handle_event(event)
                if exit_handler.detected_console:
                    config_menu.enable_console()
                    tracker.unlock_secret("activate_console")
            ...
            if config_menu.handle_event(event):
                # Não chama unlock_console_achievement aqui! Agora só no exit_handler ao digitar 'console'
                continue
        ...
        # Atualiza conquistas visíveis no menu
