        ...
        for event in pygame.event.get():
            if exit_handler.active:
                if exit_handler.handle_event(event) and exit_handler.detected_console:
                    ...
                    tracker.unlock_secret("console")   # corrigido para a ID correta
            ...
            # --- NOVO: tratar clique no ícone de upgrades sempre, mesmo quando oculto ---
            if event.type == pygame.MOUSEBUTTONDOWN:
                prev_vis = upgrade_menu.visible
                new_score, _ = upgrade_menu.handle_event(event, score)
                # se comprou ou alternou visibilidade, atualizamos e pulamos para o próximo evento
                if new_score != score or upgrade_menu.visible != prev_vis:
                    score = new_score
                    continue

                # resto do tratamento de cliques (botão principal etc.)
                button._update_rect()
                if not (
