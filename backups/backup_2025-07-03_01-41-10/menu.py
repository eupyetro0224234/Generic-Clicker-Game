...
class ConfigMenu:
    def __init__(self, screen, window_width, window_height):
        ...
        self.padding_x = 6
        self.spacing = 8  # um pouco mais de espaçamento
        ...
        # Mais próximo da borda, porém com margem confortável
        margin = 8
        self.icon_rect.topright = (window_width - margin, margin)

        self.options = ["Volume: [•••••]", "Tema: Claro", "Controles"]

        self.max_height = len(self.options) * (self.option_height + self.spacing)

        # Instancia o menu de controles
    ...
    def draw_menu(self):
        ...
        margin_right = 8  # margem direita um pouco maior
        x = self.screen.get_width() - width - margin_right
        y = self.icon_rect.bottom + 8  # distância da parte inferior do ícone
        ...
        for i, option in enumerate(self.options):
            ...
            # Alinhar o texto dentro da caixa, garantindo centralização vertical e horizontal com margem
            text_rect = text_surf.get_rect()
            text_rect.centery = option_rect.centery
            # Pequena margem lateral para não ficar colado
            text_rect.x = option_rect.x + 8  
