...
def handle_event(self, event):
    # Se submenus estão abertos, deixam eles processarem primeiro
    if self.settings_menu.visible:
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
        # Clicou no ícone do menu principal? Abre/fecha
        if self.icon_rect.collidepoint(event.pos):
            self.is_open = not self.is_open
            return True

        # Se menu principal aberto, verifica clique dentro dele
        if self.is_open and hasattr(self, "menu_rect") and self.menu_rect.collidepoint(event.pos):
            relative_y = event.pos[1] - self.menu_rect.top - 6  # vertical_padding
            index = relative_y // (self.option_height + self.spacing)
            if 0 <= index < len(self.options):
                selected = self.options[index]
                if selected == "Controles":
                    # Toggle controles, mantem menu aberto
                    self.controls_menu.visible = not self.controls_menu.visible
                    self.is_open = True
                    return True
                elif selected == "Configurações":
                    # Toggle configurações, mantem menu aberto
                    self.settings_menu.visible = not self.settings_menu.visible
                    self.is_open = True
                    return True
            # Clique dentro do menu principal mas não em uma opção válida, consome evento
            return True
        else:
            # Clique fora do menu principal (e não no ícone), fecha só menu principal
            # Mas só fecha se nenhum submenu estiver aberto
            if not self.controls_menu.visible and not self.settings_menu.visible:
                self.is_open = False
                return True
            # Se submenus estão abertos, não fecha menu principal aqui
            return False

    elif event.type == pygame.KEYDOWN:
        if event.key == pygame.K_ESCAPE:
            # Fecha submenus antes do menu principal
            if self.settings_menu.visible:
    ...
    # Se evento não for tratado, não consome (retorna False)
    return False
