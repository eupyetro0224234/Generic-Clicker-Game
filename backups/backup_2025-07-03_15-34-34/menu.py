...
class ConfigMenu:
    ...
    def draw_menu(self):
        ...
        if self.animation_progress <= 0 or self.controls_menu.visible or self.settings_menu.visible:
            return
    ...
    def handle_event(self, event):
        if self.settings_menu.visible:
            if self.settings_menu.handle_event(event):
                return True
            return False  # bloqueia eventos enquanto estiver ativo

        if self.controls_menu.visible:
            if self.controls_menu.handle_event(event):
                return True
            return False  # bloqueia eventos também
