...
class ConfigMenu:
    def __init__(self, screen, window_width, window_height, loading_callback=None):
        ...
        self.extra_icons = []  # ← ÍCONES ADICIONAIS
    ...
    def add_extra_icon(self, rect, callback):
        self.extra_icons.append((rect, callback))

    def draw_icon(self):
        # Ícone principal (configurações)
        if self.icon_image:
            self.screen.blit(self.icon_image, self.icon_rect)
        else:
            pygame.draw.rect(self.screen, (70, 130, 180), self.icon_rect)

        # Ícones extras (como upgrades)
        for rect, _ in self.extra_icons:
            pygame.draw.rect(self.screen, (100, 100, 255), rect, border_radius=6)  # pode ser substituído por imagem
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            # Ícones extras
            for rect, callback in self.extra_icons:
