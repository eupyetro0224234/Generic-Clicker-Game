...
class ConfigMenu:
    ...
    def draw(self):
        # Desenha menu principal se animado aberto
        if self.animation_progress > 0:
            self.draw_menu()
        # Desenha controles sempre que visível
        if self.controls_menu.visible:
            self.controls_menu.draw()
        # Desenha configurações sempre que visível
        if self.settings_menu.visible:
            self.settings_menu.draw()

    def handle_event(self, event):
        # Se o menu de configurações está aberto, processa evento nele
        if self.settings_menu.visible:
            if self.settings_menu.handle_event(event):
                return True

        # Se o menu de controles está aberto, processa evento nele
        if self.controls_menu.visible:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.is_open:
                ...
                if menu_rect.collidepoint(event.pos):
                    ...
                    # Clique fora fecha só menu principal
                    self.is_open = False
                    return True
