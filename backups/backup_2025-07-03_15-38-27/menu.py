...
class ConfigMenu:
    ...
    def handle_event(self, event):
        # Prioriza fechar menus internos antes do menu principal
        if self.settings_menu.visible:
            if self.settings_menu.handle_event(event):
                return True
            # Se configuracoes abertas, bloqueia o resto
            return False

        if self.controls_menu.visible:
            if self.controls_menu.handle_event(event):
                return True
            # Se controles abertos, bloqueia o resto
            return False

        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.icon_rect.collidepoint(event.pos):
                # Toggle menu principal ao clicar no ícone
            ...
            if self.is_open:
                ...
                if menu_rect.collidepoint(event.pos):
                    ...
                    if 0 <= index < len(self.options):
                        ...
                        # Não fecha o menu principal ao abrir esses submenus
                    return True
                else:
                    # Clique fora do menu fecha o menu principal, mas não os submenus
                    self.is_open = False
                    return True
