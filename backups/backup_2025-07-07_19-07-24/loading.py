...
import sys
import requests
import time  # Importado para usar o delay na mensagem final
...
class LoadingScreen:
    ...
    def draw(self, percent, message="Carregando..."):
        ...
        # Delay para mostrar a mensagem de conclusão
        if percent == 100:
            pygame.time.delay(500)  # Atraso de 500ms (0.5 segundos) para garantir que a mensagem fique visível

            # Exibe mensagem "Carregamento concluído! Iniciando o jogo..."
            done_text = self.font.render("Carregamento concluído! Iniciando o jogo...", True, self.text_color)
            done_rect = done_text.get_rect(center=(self.width // 2, self.height // 2 + 50))
            self.screen.blit(done_text, done_rect)
            pygame.display.update()

            # Delay para exibir "Concluído" e depois continuar
...
def download_assets(screen, width, height):
    ...
    loading_screen.draw(100, "Carregamento concluído!")
