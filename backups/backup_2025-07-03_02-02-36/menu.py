                ...
                if rect.collidepoint(event.pos):
                    ...
                    if 0 <= index < len(self.options):
                        ...
                        if selected == "Controles":
                            # Toggle visibilidade controles ao clicar na opção
                            self.controls_menu.visible = not self.controls_menu.visible
                            # Mantém menu principal fechado quando controles abrir
                            self.is_open = False if self.controls_menu.visible else self.is_open
                    return True
                else:
                    # Clicando fora do menu principal não fecha controles nem menu principal
                    return True
