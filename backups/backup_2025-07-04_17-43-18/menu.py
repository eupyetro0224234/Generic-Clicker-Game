...
class ConfigMenu:
    ...
    def draw_menu(self):
        ...
        width = 190
        vertical_padding = 6
        full_height = (len(self.options) + 1) * (self.option_height + self.spacing) - self.spacing + vertical_padding * 2
        ...
        # Desenha as opções "Configurações" e "Controles"
        ...
        # Desenha a opção "Sair" no final
        oy = vertical_padding + len(self.options) * (self.option_height + self.spacing)
        if oy + self.option_height <= height:
            option_rect = pygame.Rect(
                self.padding_x,
                oy,
            ...
            sair_bg = (255, 100, 100)
            sair_border = (180, 50, 50)
            pygame.draw.rect(menu_surface, sair_bg, option_rect, border_radius=self.option_radius)
            pygame.draw.rect(menu_surface, sair_border, option_rect, width=1, border_radius=self.option_radius)

            text_surf = self.font.render("Sair", True, (255, 255, 255))
            text_rect = text_surf.get_rect(center=option_rect.center)
            menu_surface.blit(text_surf, text_rect)
    ...
    def handle_event(self, event):
        # Se exit_handler ativo, passa evento para ele e ignora resto
        if self.exit_handler.active:
            if self.exit_handler.handle_event(event):
                return True

        # Primeiro deixa submenus tratar o evento
        if self.settings_menu.visible:
