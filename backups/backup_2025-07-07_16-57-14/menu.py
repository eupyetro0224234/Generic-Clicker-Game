...
class ConfigMenu:
    def __init__(self, screen, window_width, window_height, loading_callback=None):
        ...
        # controla se a opção Console está disponível no menu
    ...
    def enable_console(self):
        """Ativa a opção Console no menu (chamada a partir do ExitHandler)"""
        if not self.console_enabled:
            self.console_enabled = True
            if "Console" not in self.options:
                idx_sair = self.options.index("Sair")
    ...
    def handle_event(self, event):
        # 1) prioridade ao próprio console (se estiver aberto)
        if self.console_instance and self.console_instance.visible:
            if self.console_instance.handle_event(event):
                # se o usuário acabou de digitar "exit", o console fez self.visible=False
                # então removemos também a opção no menu:
                if not self.console_instance.visible and self.console_enabled:
                    self.console_enabled = False
                    if "Console" in self.options:
                        self.options.remove("Console")
                return True

        # 2) prioridade ao ExitHandler (quando ativo)
        if self.exit_handler.active:
            result = self.exit_handler.handle_event(event)
            # se digitou "console" no prompt de saída, habilita de novo a opção:
        ...
        # 4) clique no ícone principal (abre/fecha o menu)
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if self.is_open:
                ...
                if menu_rect.collidepoint(event.pos):
                    ...
                    # clicou fora do menu -> fecha tudo
                    self.is_open = False
                    return True

        # 5) tecla ESC fecha menus abertos
        if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
            if self.console_instance and self.console_instance.visible:
                self.console_instance.visible = False
                # e remove Console do menu
