...
from console import Console  # importando nosso console
...
def main():
    ...
    # Variável para permitir modificar score dentro do closure
    def nonlocal_set_score(new_val):
        nonlocal score
        score = new_val

    console = Console(screen, WIDTH, HEIGHT)
    # Configurar funções para acesso à pontuação no console
    console.set_score_accessors(
        get_func=lambda: score,
        set_func=nonlocal_set_score
    )

    # Configura acesso ao score no menu para habilitar o console com callbacks
    config_menu.set_score_accessors(
        get_score_func=lambda: score,
        set_score_func=nonlocal_set_score
    )
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_ESCAPE:
                    # Fechar menus e console na ordem correta
                    if console.visible:
            ...
            # Envia eventos primeiro para console e menus para bloquear ações externas
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                # Bloquear clique no botão se menus ou console estiverem abertos
                if not (
                    config_menu.settings_menu.visible or
                    config_menu.achievements_menu.visible or
                    upgrade_menu.visible or
                    console.visible
                ):
