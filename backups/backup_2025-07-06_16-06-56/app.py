...
def main():
    ...
    config_menu.achievements_menu.unlocked = set(achievements)  # define conquistas carregadas
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                # Só permite clicar no botão se nem Configurações nem Conquistas estiverem abertos
                if (not config_menu.settings_menu.visible and 
                    not config_menu.achievements_menu.visible and
                    config_menu.settings_menu.is_click_allowed(event.button)):
                    if button.is_clicked(event.pos):
                        button.click()
                        score += 1
                        click_effects.append(ClickEffect(event.pos[0], event.pos[1], "+1"))
                        # Atualiza conquistas
                        config_menu.achievements_menu.check_unlock(score)
    ...
    score_manager.save_data(score, config_menu.controls_menu.visible, list(config_menu.achievements_menu.unlocked))
