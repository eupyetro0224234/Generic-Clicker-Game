...
from config import FullSettingsMenu  # <-- IMPORTANTE
...
class ConfigMenu:
    def __init__(self, screen, window_width, window_height):
        ...
        self.full_settings_menu = FullSettingsMenu(screen, window_width, window_height)
    ...
    def draw(self):
        ...
        if self.full_settings_menu.visible:
            self.full_settings_menu.draw()
            return
        ...
        for i, option in enumerate(self.options):
            ...
            option_rect = pygame.Rect(self.padding_x, oy, width - self.padding_x * 2, self.option_height)
    ...
    def handle_event(self, event):
        if self.full_settings_menu.visible:
            return self.full_settings_menu.handle_event(event)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.is_open:
                ...
                y = self.icon_rect.bottom + 6
                height = len(self.options) * (self.option_height + self.spacing) - self.spacing + vertical_padding * 2
                ...
                if rect.collidepoint(event.pos):
                    ...
                    if 0 <= index < len(self.options):
                        ...
                        if selected == "Controles":
                            ...
                            self.full_settings_menu.visible = True
                        self.is_open = False
                        return True
                else:
                    self.is_open = False
            ...
            if event.key == pygame.K_ESCAPE:
                if self.full_settings_menu.visible:
                    self.full_settings_menu.visible = False
                    return True
                if self.controls_menu.visible:
