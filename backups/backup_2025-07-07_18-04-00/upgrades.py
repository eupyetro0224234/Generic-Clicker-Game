...
class UpgradeMenu:
    def __init__(self, screen, window_width, window_height):
        ...
        self.purchased = {}  # Agora um dict: { "upgrade_id": quantidade }

        self.upgrades = [
            Upgrade("auto_click", "Auto Clique", 5000, 0),
    ...
    def draw(self, score=0):
        ...
        for i, upg in enumerate(self.upgrades):
            ...
            color = (170, 250, 170) if self.purchased.get(upg.id, 0) > 0 else self.option_color
            ...
            count = self.purchased.get(upg.id, 0)
            label = f"{upg.name} (${upg.cost}) x{count}" if count > 0 else f"{upg.name} (${upg.cost})"
    ...
    def load_upgrades(self, upgrades: dict):
        self.purchased = upgrades.copy() if upgrades else {}
