...
# Import do módulo conquistas (assumindo conquistas.py no mesmo diretório)
...
def main():
    ...
    # Cria o menu de conquistas e carrega estado desbloqueado
    achievements_menu = AchievementsMenu(screen, WIDTH, HEIGHT)
    achievements_menu.load_achievements(unlocked_achievements)
    config_menu.achievements_menu = achievements_menu  # conecta ao config_menu
            ...
            if event.type == pygame.QUIT:
                ...
                if config_menu.settings_menu.is_click_allowed(event.button):
                    if button.is_clicked(event.pos):
                        ...
                        # Exemplo de desbloqueio: primeiro clique
                        if achievements_menu.unlock("first_click"):
                            score_manager.save_data(score, config_menu.controls_menu.visible,
                                                    achievements_menu.get_unlocked_ids())

                        # Exemplo desbloqueio 100 pontos
                        if score >= 100:
                            if achievements_menu.unlock("hundred_points"):
                                score_manager.save_data(score, config_menu.controls_menu.visible,
                                                        achievements_menu.get_unlocked_ids())
                ...
                if event.key == pygame.K_r:
                    score = 0
                    # Desbloqueio exemplo: reset score conquista
                    if achievements_menu.unlock("reset_score"):
                        score_manager.save_data(score, config_menu.controls_menu.visible,
                                                achievements_menu.get_unlocked_ids())
    ...
    # Salva score, controles e conquistas ao sair
    score_manager.save_data(score, config_menu.controls_menu.visible,
                            achievements_menu.get_unlocked_ids())
