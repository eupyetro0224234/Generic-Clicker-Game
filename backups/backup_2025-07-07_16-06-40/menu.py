...
# Import do console e função para abrir
...
class ConfigMenu:
    def __init__(self, screen, window_width, window_height, loading_callback=None):
        ...
        # Variáveis para acessar a pontuação do jogo (devem ser setadas externamente)
        self.get_score_callback = None
        self.set_score_callback = None

    def set_score_accessors(self, get_score_func, set_score_func):
        """Define as funções para pegar e alterar a pontuação do jogo, usadas pelo console"""
                    ...
                    if 0 <= idx < len(self.options):
                        ...
                        if sel == "Configurações":
                            ...
                            if self.console_instance:
                                ...
                                # Se por algum motivo console não estiver criado, tenta criar
                                if self.get_score_callback and self.set_score_callback:
                                    self.enable_console()
                            # Fecha menu ao abrir console
                        self.is_open = False
                        return True
                else:
                    self.is_open = False
