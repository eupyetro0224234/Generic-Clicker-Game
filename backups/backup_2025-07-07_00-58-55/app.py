...
def main():
    ...
    # --- UpgradeMenu posicionado no canto superior esquerdo (longe do botão) ---
    upgrade_menu = UpgradeMenu(screen, 20, 90, 220)  # largura 220, posicionado no canto
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_r:
                ...
                upgrade_menu.purchased.clear()  # reseta upgrades comprados
                continue

            # Processa cliques apenas se não está em diálogo de saída
            if event.type == pygame.MOUSEBUTTONDOWN:
                # Variável para verificar se algum elemento consumiu o clique
                event_handled = False
                ...
                # Dê prioridade ao clique no botão principal **antes** do upgrade_menu
                if not event_handled:
                    button._update_rect()  # atualiza o retângulo do botão para detectar clique corretamente
                ...
                # Se ainda não tratado, passa para upgrade_menu
                if not event_handled:
                ...
                # Por fim, verifica ícone de configurações se ainda não tratado
                if not event_handled:
        ...
        # desenha upgrade menu (posição fixa no canto superior esquerdo)
