...
from controles import ControlsMenu  # menu controles
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.icon_rect.collidepoint(event.pos):
                # Clicar no ícone abre/fecha menu principal, mas NÃO fecha controles
                ...
                if rect.collidepoint(event.pos):
                    ...
                    if 0 <= index < len(self.options):
                        ...
                        if selected == "Controles":
                            # Toggle visibilidade do menu controles ao clicar em "Controles"
                            self.is_open = False
                            self.controls_menu.visible = not self.controls_menu.visible
                        elif selected == "Configurações":
                            # Aqui você pode implementar a chamada do menu fullscreen do config.py
                            # Por enquanto, só fecha o menu pequeno
                            self.is_open = False
                    return True
                else:
                    # Clique fora fecha o menu pequeno, mas NÃO fecha controles
                    self.is_open = False
                    return True
