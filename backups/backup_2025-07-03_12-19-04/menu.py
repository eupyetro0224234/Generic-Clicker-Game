...
class ConfigMenu:
    ...
    def handle_event(self, event):
        if self.controls_menu.visible:
            # Permite ESC fechar o menu de controles
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                self.controls_menu.visible = False
                return True

            # Permite clicar dentro da caixa dos controles sem fechar
            if event.type == pygame.MOUSEBUTTONDOWN:
                if self.controls_menu.box_rect.collidepoint(event.pos):
                    return True

        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.icon_rect.collidepoint(event.pos):
                # Clicar no ícone não interfere no menu de controles
